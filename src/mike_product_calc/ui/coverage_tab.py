...
def render_coverage_tab() -> None:
            ...
            with st.spinner("计算覆盖天数中..."):
                ...
                for sku_key in weekly_sales:
                    ...
                    try:
                        ...
                        sku_dfs[sku_key] = df
                        if df.empty:
                            st.warning(f"SKU '{sku_key}' BOM 展开结果为空。")
                ...
                with st.expander("🔍 BOM 展开明细", expanded=False):
                    ...
                    st.dataframe(matrix)
                    _cat_debug = _build_material_catalog_map(sheets)
                    if _cat_debug:
                        _cat_rows = []
                        for mat in matrix.index:
                            info = _cat_debug.get(mat)
                            if info:
                                _cat_rows.append({"原料": mat, "订货单位": info["order_unit"], "单位量": info["unit_qty"]})
                        if _cat_rows:
                            st.write("**原料目录单位信息:**")
                ...
                if matrix.empty:
                    ...
                    for sku_key in weekly_sales:
                        if sku_key in sku_dfs:
                            for _, row in sku_dfs[sku_key].iterrows():
                                if row["is_gap"]:
                                    gap_materials[row["material"]] = row.get("gap_reason", "gap")

                    # 5. Compute & store results
                    ...
                    try:
                        ...
                        st.session_state.cov_sku_dicts = sku_cov.to_dict(orient="records") if not sku_cov.empty else []
                        st.session_state.cov_mat_dicts = mat_cov.to_dict(orient="records") if not mat_cov.empty else []
                        st.session_state.cov_computed = True
    ...
    diag = f"**诊断:** sheets={'✅' if sheets else '❌'} | SKUs={len(sku_options)} | 有销量={sum(1 for v in st.session_state.coverage_sales.values() if v > 0)} | 已计算={'✅' if st.session_state.get('cov_computed') else '❌'} | cov={len(st.session_state.get('cov_sku_dicts', []))}条"
    st.caption(diag)

    # ── Display results from session state ──
    if st.session_state.get("cov_computed"):
        sku_cov = pd.DataFrame(st.session_state.get("cov_sku_dicts", []))
        mat_cov = pd.DataFrame(st.session_state.get("cov_mat_dicts", []))
